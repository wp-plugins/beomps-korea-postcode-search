<?php
	...
	function bkps_custom_override_billing_fields( $fields ) {
		...
		$fields['billing_address_1']['label'] = '<input type="button" id="billing_postcode_search" value="우편번호 찾기" class="btn" onclick="openDaumPostcode();" style="height: 40px;">';
  	 	return $fields;
	}
	 ...
	 //Register Beomps Admin page
	 function register_beomps_menu_page() {
   	 	 add_menu_page('BEOMPS','BEOMPS','manage_options','beomps_menu','beomps_menu_page','',6);
 	 }
  
	 //Add Beomps Admin page
  	 function beomps_menu_page() {
   		 echo '<hr>';
  		 echo '기존 코드를 통한 옵션부분을 관리자 페이지를 통해 변경할 수 있도록 추가 예정입니다.';
  		 echo '오픈소스의 발전을 위해 많은 응원 및 지원은 언제든지 환영입니다. 감사합니다.';
  	 }